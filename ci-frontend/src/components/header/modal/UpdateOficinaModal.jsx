...
export function UpdateOficinaModal({ isOpen, onClose, onUpdate, oficina }) {
  const { dispatch } = usePageContext();
  const { state } = usePageContext();
  const [tituloValue, setTituloValue] = useState(oficina?.titulo || "");
  const [descricaoValue, setDescricaoValue] = useState(oficina?.descricao || "");
  const [imageUrlValue, setImageUrlValue] = useState(oficina?.image_url || "");
